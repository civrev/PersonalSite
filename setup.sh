#This script sets up my personal website with Azure, to be run in AzureCLI

#Makes a new resource group, and assigns the region for it
az group create \
	--name PersonalWebsite \
	--location "Central US"

#sets up the app service plan (F1 is free, but not available for linux (needed for Python))
#so this site will be available for 30 days!
#app service plan is basically setting up resource demands
az appservice plan create \
	--name personalWebServer \
	--resource-group PersonalWebsite \
	--sku B1 \
	--is-linux

#deploys a web app
#civrev is my Docker id
#flask-quickstart is the name of the container I want
#to deploy to the web app
az webapp create \
	--resource-group PersonalWebsite \
	--plan personalWebServer \
	--name cwatts \
	--deployment-container-image-name civrev/flask-quickstart

#open up internet port!
#cwatts is my name! Get your own!
az webapp config appsettings set \
	--name cwatts \
	--resource-group PersonalWebsite \
	--settings WEBSITES_PORT=8000

#blog storage for this website, up to 5gb for free trial
az storage account create \
    --access-tier Hot \
    --name cwatts \
    --resource-group PersonalWebsite \
    --location "Central US" \
    --sku Standard_LRS \
    --kind BlobStorage
